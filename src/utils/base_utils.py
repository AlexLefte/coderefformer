...
def retrieve_files_recursively(dir, suffix):
    ...
    for d in sorted(os.listdir(dir)):
        ...
        if osp.isdir(dd):
            file_lst += retrieve_files_recursively(dd, suffix)
        else:
...
def setup_paths(opt, mode):
    ...
    def save_opt_file():
        # ----------------- save options file ----------------- #
        save_dir = os.path.join(opt['exp_dir'], 'train', 'options')
        os.makedirs(save_dir, exist_ok=True)

        # Copy the options file to that location
        shutil.copy(opt['opt_path'], os.path.join(save_dir, os.path.basename(opt['opt_path'])))
    ...
    def setup_model_path():
        load_path = opt['model'].get('g_load_path')
        if not load_path:
        ...
        if model_idx == '*':
            ...
            start_iter = opt['test'].get('start_iter', 0)
            end_iter = opt['test'].get('end_iter', None)
            freq = opt['test'].get('test_freq', None)
    ...
    if mode == 'train':
        # Save opt file for experiment traceability
        save_opt_file()

...
class CodeFormerJointRefModel(SRModel):
    def feed_data(self, data):
        self.gt = data['gt'].to(self.device)
        self.input = data['in'].to(self.device)
        if 'in_large_de' in data:
        ...
        if 'latent_ref' in data:
            ...
            self.idx_ref = self.idx_ref.view(self.b, -1)
        else:
            self.idx_ref = None
        self.input_ref = data.get('in_ref')

        # Get ID embeddings
        # TODO: will do it 
        pass

        # Get facial components
        # TODO; will do it
        pass
    ...
    def init_training_settings(self):
        ...
        if self.opt.get('network_vqgan', None) is not None:  # Using this network for references
            self.hq_vqgan_fix = build_network(self.opt['network_vqgan']).to(self.device)
            self.load_network(self.hq_vqgan_fix, 'weights/vqgan_code1024.pth', strict=True, param_key='params_ema')
    ...
    def optimize_parameters(self, current_iter):
        ...
        # Generate Ref GT features and latent -> currently supports one reference. TODO: adapt for multiple refs
        B, N, C, H, W = self.input_ref.shape
        ref_flat = self.input_ref.reshape(B * N, C, H, W).to(self.device)
        x, ref_feat_dict_flat = self.hq_vqgan_fix.encoder(ref_flat, return_feats=True)
        if self.generate_idx_gt:
            _, _, quant_stats = self.hq_vqgan_fix.quantize(x)
            min_encoding_indices = quant_stats['min_encoding_indices']
            self.idx_ref = min_encoding_indices.view(self.b, N, -1)
        ref_feat_dict = {}
        ...
        if current_iter % small_per_n == 0:
            self.output, logits, lq_feat = self.net_g(self.input, w=w, detach_16=True, x_ref=ref_feat_dict)
            large_de = False
        else:
            logits, lq_feat = self.net_g(self.input_large_de, code_only=True, x_ref=ref_feat_dict)
            large_de = True
        ...
        if current_iter % 1000 == 0:
            cv2.imwrite(f'debug/{current_iter}_lr.png', tensor2img([self.input], rgb2bgr=True, min_max=(-1, 1)))
            cv2.imwrite(f'debug/{current_iter}_sr.png', tensor2img([self.output], rgb2bgr=True, min_max=(-1, 1)))
            cv2.imwrite(f'debug/{current_iter}_ref.png', tensor2img([self.input_ref[0]], rgb2bgr=True, min_max=(-1, 1)))
            cv2.imwrite(f'debug/{current_iter}_gt.png', tensor2img([self.gt], rgb2bgr=True, min_max=(-1, 1)))
        ...
        if current_iter % self.net_d_iters == 0 and current_iter > self.net_g_start_iter:
            ...
            if not large_de: # when large degradation don't need image-level loss
                ...
                # id loss
                if self.cri_id:
                    l_g_id = self.cri_id(self.output, self.gt)
                    l_g_total += l_g_id
    ...
    def test(self):
        with torch.no_grad():
            ...
            if hasattr(self, 'net_g_ema'):
                self.net_g_ema.eval()
                self.output, _, _ = self.net_g_ema(self.input, w=1, x_ref=ref_feat_dict)
            else:
                logger = get_root_logger()
                logger.warning('Do not have self.net_g_ema, use self.net_g.')
                self.net_g.eval()
                self.output, _, _ = self.net_g(self.input, w=1, x_ref=ref_feat_dict)
                self.net_g.train()
